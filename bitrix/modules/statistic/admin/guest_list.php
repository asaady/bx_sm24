...
while($arRes = $rsData->NavNext(true, "f_"))
{
	...
	if (strlen($f_FIRST_URL_FROM)>0)
	{
		if (strlen($f_FIRST_URL_FROM) >=55)
		{
			...
			$str = '<a href="'.$f_FIRST_URL_FROM.'">'.$uri_type.'://'.substr($stripped, 0, 30).'...'.substr($stripped, -10).'</a>';
		}
		else
		{
			$str = '<a href="'.$f_FIRST_URL_FROM.'">'.$f_FIRST_URL_FROM.'</a>';
		}
	...
	if (strlen($f_FIRST_URL_TO) >=55)
	{
		$stripped = preg_replace("#^(http|ftp|https|news)://(\S+)$#i", "\\2", $f_FIRST_URL_TO);
		$uri_type = preg_replace("#^(http|ftp|https|news)://(\S+)$#i", "\\1", $f_FIRST_URL_TO);

		$str .= '<a href="'.$f_FIRST_URL_TO.'"><span class="'.($f_FIRST_URL_TO_404=="Y" ? "stat_attention" : "").'">'.$uri_type.'://'.substr($stripped, 0, 30).'...'.substr($stripped, -10).'</span></a>';
	}
	else
	{
		$str .= '<a href="'.$f_FIRST_URL_TO.'"><span class="'.($f_FIRST_URL_TO_404=="Y" ? "stat_attention" : "").'">'.$f_FIRST_URL_TO.'</span></a>';
	}

	$row->AddViewField("FIRST_URL_TO", $str);
	...
	if (strlen($f_LAST_URL_LAST) >=55)
	{
		$stripped = preg_replace("#^(http|ftp|https|news)://(\S+)$#i", "\\2", $f_LAST_URL_LAST);
		$uri_type = preg_replace("#^(http|ftp|https|news)://(\S+)$#i", "\\1", $f_LAST_URL_LAST);

		$str .= '<a href="'.$f_LAST_URL_LAST.'"><span class="'.($f_LAST_URL_LAST_404=="Y" ? "stat_attention" : "").'">'.$uri_type.'://'.substr($stripped, 0, 30).'...'.substr($stripped, -10).'</span></a>';
	}
	else
	{
		$str .= '<a href="'.$f_LAST_URL_LAST.'"><span class="'.($f_LAST_URL_LAST_404=="Y" ? "stat_attention" : "").'">'.$f_LAST_URL_LAST.'</span></a>';
	}

	$row->AddViewField("LAST_URL_LAST", $str);
