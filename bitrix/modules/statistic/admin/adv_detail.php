...
$strError="";
if(AdminListCheckDate($strError, array("find_date1_period"=>$find_date1_period, "find_date2_period"=>$find_date2_period)))
{
	...
	if($value["C"]>0)
	{
		if($group=="Y")
			return	'<span title="'.htmlspecialchars($alt).'">'.
				$value["C"].($show_money=="Y"?'('.number_format($value["M"], 2, ".", "\xA0").')':'').($is_back?'<span class="required">*</span>':'').
				'</span>';
		else
			return	'<a target="_blank" title="'.htmlspecialchars($alt).'" '.
				'href="'.htmlspecialchars($url).'">'.
				$value["C"].($show_money=="Y"?'('.number_format($value["M"], 2, ".", "\xA0").')':'').'</a>'.($is_back?'<span class="required">*</span>':'');
	}
	else
		return '&nbsp;';
}
...
function create_event_list(&$lAdmin, $show_money=false, $get_total_events=false)
{
	...
	if ($GROUP=="N")
	{
		$events = CAdv::GetEventList($find_type=="id" && $find<>""?$find:$find_id,$by,$order, $arF, $v1);
	}
	elseif ($GROUP=="Y")
	{
...
<tr>
	...
	<td width="50%"><?echo number_format($f_COST, 2, ".", "\xA0");?></td>
</tr>
<tr>
	<td><span title="<?=GetMessage("STAT_OUTPUTS_ALT")?>"><?echo GetMessage("STAT_OUTPUTS")?>:</span></td>
	<td><?echo number_format($f_REVENUE, 2, ".", "\xA0");?></td>
</tr>
<tr>
	...
	if ($f_BENEFIT<0) :
		?><span class="required"><?
		echo number_format($f_BENEFIT, 2, ".", "\xA0");
		?></span><?
	else :
...
<tr>
	...
	if ($f_ROI<0) :
		...
		?><span class="stat_pointed"><?
	echo number_format($f_ROI, 2, ".", "\xA0");
		?></span><?
	endif;
	?></td>
</tr>
<tr>
	...
	<td><?echo number_format($f_SESSION_COST, 2, ".", "\xA0");?></td>
</tr>
<tr>
	<td><span title="<?=GetMessage("STAT_VISITOR_COST_ALT")?>"><?echo GetMessage("STAT_VISITOR_COST")?>:</span></td>
	<td><?echo number_format($f_VISITOR_COST, 2, ".", "\xA0");?></td>
</tr>
</table>
