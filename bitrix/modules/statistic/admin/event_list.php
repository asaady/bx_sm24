...
while($arRes = $rsData->NavNext(true, "f_")):
	...
	$strHTML=$arSites[$f_REFERER_SITE_ID].'<a href="'.$f_REFERER_URL.'">'.TruncateText(InsertSpaces($f_REFERER_URL,50,"<wbr>"),100).'</a>&nbsp;';
	$row->AddViewField("REFERER_URL", $strHTML);

	$strHTML=$arSites[$f_SITE_ID].'<a href="'.$f_URL.'">'.TruncateText(InsertSpaces($f_URL,50,"<wbr>"),100).'</a>&nbsp;';
	$row->AddViewField("URL", $strHTML);

	$strHTML='<a href="'.$f_REDIRECT_URL.'">'.TruncateText(InsertSpaces($f_REDIRECT_URL,50,"<wbr>"),100).'</a>&nbsp;';
	$row->AddViewField("REDIRECT_URL", $strHTML);

	if($STAT_RIGHT>"M")
	{
		$strHTML=($f_CHARGEBACK=="Y"?"- ":"").($f_MONEY>0?number_format($f_MONEY, 2, ".", "\xA0"):"&nbsp;");
...
if($STAT_RIGHT>"M")
	$arFooter[] = array(
		...
		"value"=>number_format($arTotal["MONEY"], 2, ".", "\xA0"),
		);
$arFooter[] = array(
