...
while($arRes = $rsData->NavNext(true, "f_"))
{
	...
	if (strlen($f_URL) >=55)
	{
		$stripped = preg_replace("#^(http|ftp|https|news)://(\S+)$#i", "\\2", $f_URL);
		$uri_type = preg_replace("#^(http|ftp|https|news)://(\S+)$#i", "\\1", $f_URL);

		$str = '<a href="'.$f_URL.'"><span class="'.($f_URL_404=="Y" ? "stat_attention" : "").'">'.$uri_type.'://'.substr($stripped, 0, 30).'...'.substr($stripped, -10).'</span></a>';
	}
	else
	{
		$str = '<a href="'.$f_URL.'"><span class="'.($f_URL_404=="Y" ? "stat_attention" : "").'">'.$f_URL.'</span></a>';
	}
