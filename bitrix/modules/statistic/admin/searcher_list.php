...
if(($arID = $lAdmin->GroupAction()) && $STAT_RIGHT=="W" && check_bitrix_sessid())
{
        ...
        foreach($arID as $ID)
        {
                ...
                case "delete":
                        @set_time_limit(0);
                        $DB->StartTransaction();
			$DB->Query("DELETE FROM b_stat_searcher WHERE ID = ".$ID, false, $err_mess.__LINE__);
			$DB->Query("DELETE FROM b_stat_searcher_params WHERE SEARCHER_ID = ".$ID, false, $err_mess.__LINE__);
			$DB->Query("DELETE FROM b_stat_searcher_day WHERE SEARCHER_ID = ".$ID, false, $err_mess.__LINE__);
                        $DB->Commit();
                        break;
                }
        }
