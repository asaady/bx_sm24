...
if(AdminListCheckDate($lAdmin, array("find_date1_period"=>$find_date1_period, "find_date2_period"=>$find_date2_period)))
{
	$arFilter = Array(
...
while($arRes = $rsData->NavNext(true, "f_")):
	...
	if($STAT_RIGHT>"M"):
		$row->AddViewField("COST", number_format($f_COST, 2, ".", "\xA0"));
		$row->AddViewField("REVENUE", number_format($f_REVENUE, 2, ".", "\xA0"));
		$row->AddViewField("BENEFIT", number_format($f_BENEFIT, 2, ".", "\xA0"));
		$row->AddViewField("ROI", $f_ROI<0?"-":number_format($f_ROI, 2, ".", "\xA0"));
		$row->AddViewField("SESSION_COST", number_format($f_SESSION_COST, 2, ".", "\xA0"));
		$row->AddViewField("VISITOR_COST", number_format($f_VISITOR_COST, 2, ".", "\xA0"));
	endif;
