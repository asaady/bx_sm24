...
while($arRes = $rsData->NavNext(true, "f_"))
{
	...
	if (strlen($f_URL_LAST) >=50)
	{
		$stripped = preg_replace("#^(http|ftp|https|news)://(\S+)$#i", "\\2", $f_URL_LAST);
		$uri_type = preg_replace("#^(http|ftp|https|news)://(\S+)$#i", "\\1", $f_URL_LAST);

		$str = "<a target=\"_blank\" href=\"".$f_URL_LAST."\" ".($f_URL_LAST_404 == "Y" ? "class=\"stat_attention\"" : "").">".InsertSpaces($uri_type."://".substr($stripped, 0, 30)."...".substr($stripped, -10),22, "<br>")."</a>";
	}
	else
	{
		$str = "<a target=\"_blank\" href=\"".$f_URL_LAST."\" ".($f_URL_LAST_404 == "Y" ? "class=\"stat_attention\"" : "").">".InsertSpaces($f_URL_LAST, 22, "<br>")."</a>";
	}

	$row->AddViewField("URL_LAST", $str);


	if (strlen($f_URL_FROM) > 0)
	{
		if (strlen($f_URL_FROM) >=50)
		{
			$stripped = preg_replace("#^(http|ftp|https|news)://(\S+)$#i", "\\2", $f_URL_FROM);
			$uri_type = preg_replace("#^(http|ftp|https|news)://(\S+)$#i", "\\1", $f_URL_FROM);

			$str = "<a target=\"_blank\" href=\"".$f_URL_FROM."\">".InsertSpaces($uri_type."://".substr($stripped, 0, 30)."...".substr($stripped, -10), 22, "<br>")."</a>";
		}
		else
		{
			$str = "<a target=\"_blank\" href=\"".$f_URL_FROM."\">".InsertSpaces($f_URL_FROM, 22, "<br>")."</a>";
		}

		$row->AddViewField("URL_FROM", $str);
	}


	if (strlen($f_FIRST_URL_FROM) > 0)
	{
		if (strlen($f_FIRST_URL_FROM) >=50)
		{
			$stripped = preg_replace("#^(http|ftp|https|news)://(\S+)$#i", "\\2", $f_FIRST_URL_FROM);
			$uri_type = preg_replace("#^(http|ftp|https|news)://(\S+)$#i", "\\1", $f_FIRST_URL_FROM);

			$str = '<a target="_blank" href="'.$f_FIRST_URL_FROM.'">'.InsertSpaces($uri_type.'://'.substr($stripped, 0, 30).'...'.substr($stripped, -10),20,"<br>").'</a>';
		}
		else
		{
			$str = '<a target="_blank" href="'.$f_FIRST_URL_FROM.'">'.InsertSpaces($f_FIRST_URL_FROM,20,"<br>").'</a>';
		}

		$row->AddViewField("FIRST_URL_FROM", $str);
	}
