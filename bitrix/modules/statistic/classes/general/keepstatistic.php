...
class CKeepStatistics
{
	...
	function ReallyKeep()
	{
		global $DB, $USER, $APPLICATION, $SESS_AUTH, $FAVORITES, $HIT_ID, $STOP_SAVE_STATISTIC, $STOP_MESSAGE, $STOP_REDIRECT_URL, $STOP, $STOP_LIST_ID, $STOP_MESSAGE_LID, $event1, $event2, $event3, $goto, $money, $currency, $phrase_id, $last_referer1, $last_referer2, $referer1, $referer2, $referer3, $I2C_ERROR, $site_id;
				...
				************************************************/
				...
				else // it is not searcher
				{
					...
					************************************************/
					...
					$PAGE_FILE_TYPE		= __GetFullFileType($CURRENT_PAGE); // page type
					...
					/*
					echo "CURRENT_PROTOCOL = ".$CURRENT_PROTOCOL."<br>";
					echo "CURRENT_PORT = ".$CURRENT_PORT."<br>";
					echo "CURRENT_HOST = ".$CURRENT_HOST."<br>";
					echo "PAGE_FILE_TYPE = ".$PAGE_FILE_TYPE."<br>";
					echo "CURRENT_PAGE = ".$CURRENT_PAGE."<br>";
					echo "CURRENT_URI = ".$CURRENT_URI."<br>";
					...
					************************************************/
					...
					if (intval($_SESSION["SESS_SESSION_ID"])<=0)
					{
						...
						$_SESSION["SESS_SESSION_ID"] = $DB->Insert("b_stat_session",$arFields, "File: ".__FILE__."<br>Line: ".__LINE__);
						$_SESSION["SESS_SESSION_ID"] = intval($_SESSION["SESS_SESSION_ID"]);
						if ($ERROR_404=="N")
						{
							CStatistics::Set404("b_stat_session", "ID = ".intval($_SESSION["SESS_SESSION_ID"]), array("URL_TO_404" => "Y", "URL_LAST_404" => "Y"));
						}
						...
						$arFields = Array(
							"SESSIONS" => "SESSIONS + 1",
							"LAST_SESSION_ID" => intval($_SESSION["SESS_SESSION_ID"]),
						...
						if ($_SESSION["SESS_GUEST_NEW"]=="Y") $arFields["FIRST_SESSION_ID"] = intval($_SESSION["SESS_SESSION_ID"]);
						...
						if ($SAVE_REFERERS!="N")
						{
							...
							************************************************/
							...
							// получим данные ссылающегося сайта
							if (__GetReferringSite($PROT, $SN, $SN_WithoutPort, $PAGE_FROM))
							{
								//если это не тот хост где мы сейчас
								if (strlen($SN)>0 && $SN!=$_SERVER["HTTP_HOST"])
								{
									// определим ID домена с которого к нам пришли
									$strSql = "SELECT ID FROM b_stat_referer WHERE SITE_NAME = '".$DB->ForSql($SN,255)."'";
									$z = $DB->Query($strSql, false, "File: ".__FILE__."<br>Line: ".__LINE__);
									$zr = $z->Fetch();
									$_SESSION["SESS_REFERER_ID"] = intval($zr["ID"]);
									// если домен найден то
									if ($_SESSION["SESS_REFERER_ID"]>0)
									{
										// обновляем у него счетчик сессий
										$DB->Update("b_stat_referer",Array("SESSIONS" => "SESSIONS + 1"),"WHERE ID=".$_SESSION["SESS_REFERER_ID"],"File: ".__FILE__."<br>Line: ".__LINE__,false,false,false);
									}
									else
									{
										// иначе добавляем его в базу
										$arFields = Array(
											"DATE_FIRST"	=> $DB_now,
											"DATE_LAST"		=> $DB_now,
										...
										$_SESSION["SESS_REFERER_ID"] = $DB->Insert("b_stat_referer",$arFields, "File: ".__FILE__."<br>Line: ".__LINE__);
									}

									// добавим в базу ссылающийся сайт
									$arFields = Array(
										"DATE_HIT" => $DB_now,
										"REFERER_ID" => intval($_SESSION["SESS_REFERER_ID"]),
									...
									$REFERER_LIST_ID = $DB->Insert("b_stat_referer_list",$arFields, "File: ".__FILE__."<br>Line: ".__LINE__);
									if ($ERROR_404=="N")
									{
										CStatistics::Set404("b_stat_referer_list", "ID = ".intval($REFERER_LIST_ID), array("URL_TO_404" => "Y"));
									}
					...
					************************************************/
					...
					if (intval($_SESSION["SESS_SESSION_ID"])>0)
					{
						...
						************************************************/
						...
						$DB->Update("b_stat_session",$arFields,"WHERE ID=".intval($_SESSION["SESS_SESSION_ID"]), "File: ".__FILE__."<br>Line: ".__LINE__,false,false,false);
						if ($ERROR_404=="N")
						{
							CStatistics::Set404("b_stat_session", "ID = ".intval($_SESSION["SESS_SESSION_ID"]), array("URL_LAST_404" => "Y"));
						}
						...
						// если данный хит был сделан из результатов поиска то
						if ($SAVE_REFERERS!="N" && intval($phrase_id)>0)
						{
							// обновляем таблицу поисковых фраз
							$arFields = Array(
								"URL_TO"		=> "'".$DB->ForSql($CURRENT_URI,2000)."'",
								"URL_TO_404"	=> "'".$ERROR_404."'"
								);
							$DB->Update("b_stat_phrase_list",$arFields, "WHERE ID='".intval($phrase_id)."' and SESSION_ID=".intval($_SESSION["SESS_SESSION_ID"]), "File: ".__FILE__."<br>Line: ".__LINE__);
							if ($ERROR_404=="N")
							{
								CStatistics::Set404("b_stat_phrase_list", "ID='".intval($phrase_id)."' and SESSION_ID='".$_SESSION["SESS_SESSION_ID"]."'", array("URL_TO_404" => "Y"));
							}
						}
					...
					if ($PAGE_FILE_TYPE=="SOURCE")
					{
						...
						*******************************************************/
						...
						/*
						echo "SESS_HTTP_REFERER = ".$_SESSION["SESS_HTTP_REFERER"]."<br>";
						echo "SESS_LAST_PROTOCOL = ".$_SESSION["SESS_LAST_PROTOCOL"]."<br>";
						echo "SESS_LAST_PORT = ".$_SESSION["SESS_LAST_PORT"]."<br>";
						echo "SESS_LAST_HOST = ".$_SESSION["SESS_LAST_HOST"]."<br>";
						echo "SESS_LAST_URI = ".$_SESSION["SESS_LAST_URI"]."<br>";
						echo "SESS_LAST_PAGE = ".$_SESSION["SESS_LAST_PAGE"]."<br>";
						echo "SESS_LAST_DIR = ".$_SESSION["SESS_LAST_DIR"]."<br>";
						*/
	...
	function SavePathData($SITE_ID, $CURRENT_PAGE, $ERROR_404)
	{
		...
		else
		{
			...
			if(strtolower($DB->type)=="oracle")
				$arrUpdate404_1["PATH_PAGES"] = "'".$DB->ForSql($CURRENT_PATH_PAGES_404, 2000)."'";
			elseif(strtolower($DB->type)=="mssql")
				$arrUpdate404_1["PATH_PAGES"] = "'".$DB->ForSql($CURRENT_PATH_PAGES_404, 7000)."'";
			else
				$arrUpdate404_1["PATH_PAGES"] = "'".$DB->ForSql($CURRENT_PATH_PAGES_404)."'";
			$arrUpdate404_2["PAGES"] = "'".$CURRENT_PATH_PAGES_404."'";
		}
