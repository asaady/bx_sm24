...
<?if (is_array($arResult["PROPERTY_LIST"]) && !empty($arResult["PROPERTY_LIST"])):?>
	...
	<div class="form-group">
		...
		?>
			<input type="hidden" name="PROPERTY_OLD[<?=$propertyID?>][<?=$i?>]"value="<?=$value?>" />
		<?}
		...
		switch ($INPUT_TYPE):
			...
			case "T":
				...
				?>
			<textarea class="form-control" id="<?=$propertyID?>_INPUT" cols="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]?>" rows="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["ROW_COUNT"]?>" name="PROPERTY[<?=$propertyID?>][<?=$i?>]"<?if($arResult["PROPERTY_LIST_FULL"][$propertyID]["DISABLED"] == "Y"):?> readonly<?endif?>><?=$value?></textarea>
				<?
				}
			break;
			...
			case "N":
				...
				?>
				<input class="form-control"<?if($i==0):?> id="<?=$propertyID?>_INPUT"<?endif?> type="text" name="PROPERTY[<?=$propertyID?>][<?=$i?>]"<?if($arResult["PROPERTY_LIST_FULL"][$propertyID]["DISABLED"] == "Y"):?> readonly<?endif?> size="25" value="<?=$value?>" />
				...
				<input class="form-control inclone__block" type="text" name="PROPERTY[<?=$propertyID?>][]" size="25" value="" />
				...
				switch ($type):
					...
					case "radio":
						foreach ($arResult["PROPERTY_LIST_FULL"][$propertyID]["ENUM"] as $key => $arEnum)
						{
							...
							?>
			<input type="<?=$type?>" name="PROPERTY[<?=$propertyID?>]<?=$type == "checkbox" ? "[".$key."]" : ""?>"<?if($arResult["PROPERTY_LIST_FULL"][$propertyID]["DISABLED"] == "Y"):?> readonly<?endif?> value="<?=$key?>" class="form-control" id="property_<?=$key?>"<?=$checked ? " checked=\"checked\"" : ""?> /><label for="property_<?=$key?>"><?=$arEnum["VALUE"]?></label><br />
							<?
						}
					break;
					...
					case "multiselect":
					?>
			<select class="form-control" id="<?=$propertyID?>_INPUT" name="PROPERTY[<?=$propertyID?>]<?=$type=="multiselect" ? "[]" : ""?>"<?if($arResult["PROPERTY_LIST_FULL"][$propertyID]["DISABLED"] == "Y"):?> readonly<?endif?> <?=$type=="multiselect" ? "size=\"".$arResult["PROPERTY_LIST_FULL"][$propertyID]["ROW_COUNT"]."\" multiple=\"multiple\"" : ""?>>
...
<?if($arParams["USE_CAPTCHA"] == "Y" && $arParams["ID"] <= 0):?>
	<div class="form-group">
		<label for="<?=$propertyID?>_INPUT" class="col-sm-3 control-label">Текст с картинки<span class="starrequired">*</span></label>
		<div class="col-sm-9">
	...
	<div class="form-group">
		...
		<div class="col-md-9">
			...
			<?if (strlen($arParams["LIST_URL"]) > 0):?>
				<input class="btn btn-primary" type="submit" name="iblock_submit" value="Сохранить" />
			<?endif?>
				<input class="btn btn-primary" type="submit" name="iblock_apply" value="Применить" />
			<?if (strlen($arParams["LIST_URL"]) > 0):?>
