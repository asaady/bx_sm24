...
if(!empty($_REQUEST["q"]) && CModule::IncludeModule("catalog") && CModule::IncludeModule("yadadya.shopmate"))
{
	...
	$egais = CIBlockSection::GetList(array(), array("IBLOCK_ID" => 2, "SECTION_ID" => 3));
	while($arEgais = $egais->GetNextElement())
	{
		$egaisSecFields = $arEgais->GetFields();
		$egaisFilter[] = $egaisSecFields["ID"];
	}
	$arFilter["!SECTION_ID"] = $egaisFilter;
