		...
		<div class="mb20 inclone">
			...
			<?foreach($arResult["PROPERTY_LIST_FULL"][$propertyID]["ENUM"] as $arDocElement):
				...
				<tr class="<?if($arDocElement["SAMPLE"] == "Y"):?>inclone__block <?else:?>deleted_block <?endif?>product_block">
					<td><?if($arDocElement["SAMPLE"] != "Y"):?><input type="hidden" class="product_basket_id" name="PROPERTY<?=$inputPrefix?>[<?=$propertyID?>][ID][]" value="<?=$arElemParams["ID"]?>"><?=$arElemParams["ID"]?>.<?endif?></td>
				<?foreach ($arResult["PROPERTY_LIST_FULL"][$propertyID]["arResult"]["PROPERTY_LIST"] as $colkey => $propID):?>
					<td>
...
<form class="panel panel-body save_alert" name="iblock_add" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data" id="egais">
	<div class="mb30">
		<h1>Входящая ТТН №<?=$arResult["ELEMENT"]["NUMBER"]?> от <?=$arResult["ELEMENT"]["DATE"]?></h1>
	</div>
	<?=bitrix_sessid_post()?>
...
<?endif?>
	...
	<?//if($arResult["ELEMENT"]["ACCEPTED"] != "Y"):?>
...
<?foreach ($arResult["ACT_ITEMS"] as $arAct):?>
<?=$arAct["NAME"]?> [<?=$arAct["DATE"]?>]:<br />
<?=$arAct["NOTE"]?><br /><br />
<?endforeach?>
