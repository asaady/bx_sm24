	...
	<textarea class="form-control" id="<?=$propertyID?>_INPUT" cols="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]?>" rows="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["ROW_COUNT"]?>" name="PROPERTY<?=$inputPrefix?>[<?=$propertyID?>][]"<?if($arResult["PROPERTY_LIST_FULL"][$propertyID]["DISABLED"] == "Y"):?> readonly<?endif?>><?=$value?></textarea>
		...
		<div class="input-group">
			...
			<input class="form-control <?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["CLASS"]?>"
				...
				value="<?=$value?>" />	
			<?if($arResult["PROPERTY_LIST_FULL"][$propertyID]["USER_TYPE"] == "DateTime"):?><span class="input-group-addon" onclick="BX.calendar({node:this, field:previousElementSibling, form: '', bTime: false, currentTime: '<?=(time()+date("Z")+CTimeZone::GetOffset())?>', bHideTime: true});"><i class="glyphicon glyphicon-calendar"></i></span><?endif?>
			<?}?>
			...
			switch ($type):
				...
				case "radio":
					...
					foreach ($arResult["PROPERTY_LIST_FULL"][$propertyID]["ENUM"] as $key => $arEnum)
					{
						...
						?>
		<input type="<?=$type?>" name="PROPERTY<?=$inputPrefix?>[<?=$propertyID?>]<?=$type == "checkbox" ? "[".$key."]" : ""?>"<?if($arResult["PROPERTY_LIST_FULL"][$propertyID]["DISABLED"] == "Y"):?> readonly<?endif?> value="<?=$key?>" id="property_<?=$propertyID?>_<?=$key?>"<?=$checked ? " checked=\"checked\"" : ""?> /><label for="property_<?=$propertyID?>_<?=$key?>"><?=$arEnum["VALUE"]?></label><br />
						<?
					}
				break;
				...
				case "multiselect":
				?>
		<select class="form-control w100p<?if($type=="ajax"):?> ajax_select<?endif?> <?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["CLASS"]?>"<?if($type=="ajax"):?> data-url="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["URL"]?>"<?endif?><?if(strlen($arResult["PROPERTY_LIST_FULL"][$propertyID]["INFO_URL"])):?> data-info_url="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["INFO_URL"]?>"<?endif?> id="<?=$propertyID?>_INPUT" name="PROPERTY<?=$inputPrefix?>[<?=$propertyID?>]<?=$type=="multiselect" || $type=="ajax" ? "[]" : ""?>"<?if($arResult["PROPERTY_LIST_FULL"][$propertyID]["DISABLED"] == "Y"):?> readonly<?endif?><?if($arResult["PROPERTY_LIST_FULL"][$propertyID]["MULTIPLE"] == "Y"):?> size="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["ROW_COUNT"]?>" multiple="multiple"<?endif?> placeholder="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"]?>"<?if(is_array($arResult["PROPERTY_LIST_FULL"][$propertyID]["DATA"])) foreach($arResult["PROPERTY_LIST_FULL"][$propertyID]["DATA"] as $dname => $dval) echo " data-".$dname."=\"".$dval."\"";?>>
...
<form action="<?=POST_FORM_ACTION_URI?>" method="get" enctype="multipart/form-data" class="cmpfilter">
	...
	<div class="row">
		<div class="col-md-4">
			<div class="btn-list clearfix">
				За текущий
				<a class="btn btn-info" href="<?=$arResult["CURRENT_MONTH"]?>">месяц</a>
				<a class="btn btn-info" href="<?=$arResult["CURRENT_QUARTER"]?>">квартал</a>
				<a class="btn btn-info" href="<?=$arResult["CURRENT_YEAR"]?>">год</a>
			</div>
		</div>
		<div class="col-md-6">
		</div>
