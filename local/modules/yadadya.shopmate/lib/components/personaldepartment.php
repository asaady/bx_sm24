...
class PersonalDepartment extends Base
{
	...
	public static function getPropList()
	{
		...
		$res = Shopmate\Internals\PersonalDepartmentTable::getList(array("select" => array("ID", "NAME", "PARENT_ID")));
		while ($row = $res->fetch())
			$parentList[] = $row;
	...
	public function getList(array $parameters = array())
	{
		...
		return Shopmate\Internals\PersonalDepartmentTable::getList($parameters);
	}
	...
	{
		$result = Shopmate\Internals\PersonalDepartmentTable::getList(array("filter" => array("ID" => $primary)))->fetch();
		return $result;
	}
	...
	public function update($primary, array $data, $result = null)
	{
		...
		{
			if ($primary > 0)
				$res = Shopmate\Internals\PersonalDepartmentTable::update($primary, $data);
			else
			{
				$data["STORE_ID"] = Shopmate\Shops::getUserStore();
				$res = Shopmate\Internals\PersonalDepartmentTable::add($data);
				if ($res->isSuccess())
					$primary = $res->GetID();
			}
	...
	public function delete($primary)
	{
		...
		$result = Shopmate\Internals\PersonalDepartmentTable::delete($primary);
	...
	public function getEnumList(array $filter = array(), $json = false)
	{
		...
		$result = Shopmate\Internals\PersonalDepartmentTable::GetList(array(
