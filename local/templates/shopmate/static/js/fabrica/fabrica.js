...
$.fn.fabrica_form = function() {
	...
	var $form = this,
		...
		$item_add_btn = $form.find('.fabrica_form__item_add'),
		$item_sample = $items_block.find('.fabrica_form__item.fabrica_form__item__sample:first').clone(),
		items_list = {},
		simple = $form.hasClass('fabrica_form__simple'),
		network = {edges: {}, nodes: {}},
		type_list = {
	...
	var that = {
		...
		itemInterface: function($item_added){
			...
			var $type_inputs = $item_added.find('input[data-type]'),
				...
				$item_del_btn = $item_added.find('.item_del'),
				$item_id = $item_added.find('.item_id');
			...
			if(Object.keys(items_list).length <= 0) {
				$item_del_btn.remove();
				$type_inputs.filter('[data-type='+type_list.simple+']').hide();
				$type_inputs.filter('[data-type='+type_list.simple+']').parent('label').hide();
			}
			else {
				$item_del_btn.click(function() {
					$item_added.modal('hide');
					$items_block.each(function() {
						$(this).find('.item_connects .item_connect').each(function() {
							if($(this).find('.item_connect__id').val() == $item_id.val()) {
								$(this).remove();
							}
						});
					});
					delete items_list[$item_id.val()];
					$item_added.remove();
					that.updateGraph();
				});
			}
		...
		productSearch: function($item, prefix) {
			...
			$find.hide();
			prodSelectTab($name, $search);
			$search.data('select2').on('query', function() {
				search_text = $search.data('select2').$container.find('.select2-search__field').val();
				$name.val(search_text).change();
			});
			$search.data('select2').on('close', function(event) {
				$search.data('select2').$container.find('.select2-search__field').val(search_text);
			});
			$search.on("select2:select", function (e) { 
				$search.data('select2').$container.find('.select2-search__field').val('');
				$name.val($search.find('[value='+$search.val()+']').html()).change();
			});
			$search.on("change", function (e) { 
				if($search.val() <= 0 || $search.val() == null || $search.val() == undefined) {
					$search.html('');
					$search.data('select2').$container.find('.select2-search__field').val('');
				}
				$product_id.val($search.val());
		...
		reMeasure: function($item_added) {
			...
			that.reMeasureConnectChange($item_added.find('.item_measure_translate'));
